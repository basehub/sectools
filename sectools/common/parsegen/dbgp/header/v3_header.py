...
class DbgpHdrv3(DbgpHdrv2):
    ...
    def unpack(self, data, offset=0):
        ...
        # Unpack number of rootcerthashes_qc
        fmt = self.get_format_rootcerthash_qc_count()
        if self.data_is_none:
            data = '\0' * struct.calcsize(fmt)
        self._unpack_rootcerthash_qc_count(struct.unpack_from(fmt, data))
        data = data[struct.calcsize(fmt):]

        # Unpack the hashes_qc
        fmt = self.get_format_rootcerthash_qc_array()
        if self.data_is_none:
            data = '\0' * struct.calcsize(fmt)
        self._unpack_rootcerthash_qc_array(struct.unpack_from(fmt, data))
        data = data[struct.calcsize(fmt):]
    ...
    def _unpack_rootcerthash_qc_count(self, unpacked):
        self._rootcerthash_qc_count = unpacked[0]

    def _unpack_rootcerthash_qc_array(self, unpacked):
        self._rootcerthash_qc_array = list(unpacked)
        for i in range(self.rootcerthash_qc_count):
            self._rootcerthash_qc_array[i] = binascii.hexlify(self._rootcerthash_qc_array[i])
    ...
    def pack(self):
        ...
        return (self._pack_base() +
                ...
                self._pack_rootcerthash_qc_count() +
                self._pack_rootcerthash_qc_array()
                )

    def _pack_rootcerthash_qc_count(self):
        values = [self.rootcerthash_qc_count]
        return struct.pack(self.get_format_rootcerthash_qc_count(), *values)

    def _pack_rootcerthash_qc_array(self):
        values = []
        for i in range(self.rootcerthash_qc_count):
            values.append(binascii.unhexlify(self.rootcerthash_qc_array[i]))
        values = values + list([''] * (ROOTCERTHASH_MAX_COUNT - self.rootcerthash_qc_count))
        return struct.pack(self.get_format_rootcerthash_qc_array(), *values)

    @classmethod
    def get_format_rootcerthash_qc_count(cls):
        return '<I'

    @classmethod
    def get_format_rootcerthash_qc_array(cls):
    ...
    def validate_ranges(self):
        ...
        if not (0 <= self.rootcerthash_qc_count <= ROOTCERTHASH_MAX_COUNT):
            raise RuntimeError('Invalid number of rootcert hashes supplied in config file. ' +
                               '0 - ' + str(ROOTCERTHASH_MAX_COUNT) + ' hashes supported, ' +
                               str(self.rootcerthash_qc_count) + ' detected.')
    ...
    def _repr_properties(self):
        ...
        properties = properties + [('Root Certificate Hash QC Count', self.rootcerthash_qc_count)]
        if self.rootcerthash_qc_count != 0:
            for i in self.rootcerthash_qc_array:
                properties = properties + [('Root Certificate Hash QC', i)]

        return [(attr, val) for attr, val in properties]

    @property
    def rootcerthash_qc_count(self):
        return self._rootcerthash_qc_count

    @property
    def rootcerthash_qc_array(self):
        return self._rootcerthash_qc_array

    @rootcerthash_qc_array.setter
    def rootcerthash_qc_array(self, value):
        for i in range(len(value)):
            if len(value[i]) != 64:
                raise RuntimeError('rootcerthash_qc is not equal to 256 bits, rxd input rootcerthash_qc bit length as '+str(len(value[i])*4))

        self._rootcerthash_qc_count = len(value)
        self._rootcerthash_qc_array = value
    ...
    def data_size(self):
        return (struct.calcsize(self.get_format()) +
                ...
                struct.calcsize(self.get_format_rootcerthash_qc_count()) +
                struct.calcsize(self.get_format_rootcerthash_qc_array())
               )
